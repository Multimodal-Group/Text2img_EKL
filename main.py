...
if __name__ == "__main__":
    ...
    output_dir = './output/%s_%s_%s' % \
        (cfg.DATASET_NAME, cfg.CONFIG_NAME, timestamp)  ## ./output/birds_3stages_time

    split_dir, bshuffle = 'train', True
    if not cfg.TRAIN.FLAG:
        if cfg.DATASET_NAME == 'birds' or cfg.DATASET_NAME=='flowers':
            #bshuffle = False  ## origin order gen
            #split_dir = 'train'
            split_dir = 'test'

    # Get data loader
    imsize = cfg.TREE.BASE_SIZE * (cfg.TREE.SCALE ** (cfg.TREE.BRANCH_NUM-1))
    print('Top-stack images size: %dx%d'%(imsize,imsize))
    ...
    if cfg.DATA_DIR.find('lsun') != -1:
        ...
        dataset = TextDataset(cfg.DATA_DIR, split_dir,
                              ...
                              transform=image_transform)
    assert dataset
    num_gpu = len(cfg.GPU_ID.split(','))  ## --gpu 0,1,2,3
    dataloader = torch.utils.data.DataLoader(
        dataset, batch_size=cfg.TRAIN.BATCH_SIZE * num_gpu,  ## bs= 24x4
        drop_last=True, shuffle=bshuffle, num_workers=int(cfg.WORKERS))  # 4 workers
    ...
    if not cfg.GAN.B_CONDITION:
        ...
        # from cub_trainer import condGANTrainer as trainer
        # print('USE CUB TRAINER')
        # from cub_trainer_capsule import condGANTrainer as trainer
        # from cub_trainer_splitz import condGANTrainer as trainer
        from cub_trainer_splitz_cap_ca import condGANTrainer as trainer
        print('USE CUB TRAINER by splitz with capsule with ca')
